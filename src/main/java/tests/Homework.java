...
public class Homework {
    /**
     * 1. Создать аннотации Test, BeforeEach, BeforeAll, AfterEach, AfterAll
     * 2. Создать класс TestRunner, который принимает класс и запускает "тесты"
     * 3. Тест - это метод, над которым стоит аннотация Test
     * 4. Перед всеми тестами запускаются методы, над которыми стоит BeforeAll
     * 5. Перед каждым тестом запускаются методы, над которыми стоит BeforeEach
     * 6. Аналогично для AfterEach, AfterAll
